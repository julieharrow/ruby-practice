# Future changes:
# 1. Include logic that favors Ace as 1 or 11 depending on the situation
# 2. Include logic to handle user error and unexpected input
...
def shuffle deck, shuffled_deck
  ...
  chosen_card = deck.fetch(rand(num_cards-1))
  shuffled_deck.push(chosen_card)
  deck = deck - [chosen_card]
  shuffle(deck, shuffled_deck)
  shuffled_deck
end

def deal(deck, hand, num_cards_to_deal)
  count = 0
  while count < num_cards_to_deal
    dealt_hand = hand.push(deck[count])
    count+=1
  end
  deck.shift(num_cards_to_deal)
  return deck, dealt_hand
end
...
def value_of_hand(hand,deck_values)
  ...
  (0..num_cards-1).each do |x|
    value = value + deck_values[hand[x]]
    x+=1
  end
  return value
end
...
(0..num_players-1).each do |x|
  ...
  player_names = player_names.push(gets.chomp.capitalize.strip)
  puts "What is player #{x+1}'s bet?"
  player_bets = player_bets.push(gets.chomp.to_i)
