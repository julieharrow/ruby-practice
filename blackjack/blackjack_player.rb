...
class Player
  attr_reader :hand, :score, :bank, :status
  attr_accessor :name, :bet, :directive
  def initialize(name,bet,hand,score,bank,status,directive)
    @name,@bet,@hand,@score,@bank,@status,@directive = name,bet,hand,score,bank,status,directive
  end
end
